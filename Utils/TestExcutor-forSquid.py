...
def recoverDefaultConfigFile():
	...
	if Software == 'Squid':
		...
		try:
			move(SquidConfigDir+"_bak", SquidConfigDir)
		except IOError:
			print "recoverDefaultConfigFile Error!", Software
...
def updateDefaultConfigFile(key, value):
	...
	if Software == 'Squid':
		SquidConfigDir = "/etc/squid/squid.conf"
		try:
			with open( SquidConfigDir, 'r' ) as fp:
				lines = fp.readlines()
				lines = replacewith(lines, key, value)

			with open( SquidConfigDir+"_tmp", 'w' ) as fp:
				fp.writelines( lines )
			move(SquidConfigDir, SquidConfigDir+"_bak")
			move(SquidConfigDir+"_tmp", SquidConfigDir)
		except IOError:
...
def excuteOneTestWithConfig(config, value, oneTestDir, oneTest):
	...
	if Software == 'Squid':
		...
		ResultDir = getResultDir(config, oneTest)
		if os.path.exists(ResultDir) == False: os.mkdir(ResultDir)
		CMD = "pwd; mv /usr/local/squid/t/oc.txt " + "  "+ ResultDir + oneTest.replace(".t", "_StartWith_"+str(value)+".result")
		print CMD
		os.system(CMD)

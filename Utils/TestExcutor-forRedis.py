...
def recoverDefaultConfigFile():
	...
	if Software == 'Redis':
		RedisConfigDir = "/redis-6.0.0/tests/assets/default.conf"
		try:
			move(RedisConfigDir+"_bak", RedisConfigDir)
		except IOError:
			print "recoverDefaultConfigFile Error!", Software
...
def updateDefaultConfigFile(key, value):
	...
	if Software == 'Redis':
		RedisConfigDir = "/redis-6.0.0/tests/assets/default.conf"
		try:
			with open( RedisConfigDir, 'r' ) as fp:
				lines = fp.readlines()
				lines.append("\n\n"+str(key) + " " + str(value) + "\n")

			with open( RedisConfigDir+"_tmp", 'w' ) as fp:
				fp.writelines( lines )
			move(RedisConfigDir, RedisConfigDir+"_bak")
			move(RedisConfigDir+"_tmp", RedisConfigDir)
		except IOError:
			print "updateDefaultConfigFile Error!"
...
def excuteOneTestWithConfig(config, value, oneTestDir, oneTest):
	...
	if Software == 'Redis':
		OriginalTestName = getOriginalTestName(oneTest) #xx.tcl
		TestToolDir = "/redis-6.0.0/"
		TestCaseDir = TestToolDir + "tests/RedisTest/"
		#test_helper.tcl
		...
		CMD = "python changeTest.py " + TestToolDir+"tests/test_helper.tcl" + " " + OriginalTestName.replace(".tcl", "") + " ; "
		print CMD
		os.system(CMD)
		

		CMD = "cd " + TestToolDir + "; "
		CMD = CMD + "./runtest | tee oc.txt ; "
		print CMD
		os.system(CMD)

		ResultDir = getResultDir(config, oneTest)
		if os.path.exists(ResultDir) == False: os.mkdir(ResultDir)
		CMD = "pwd; mv /redis-6.0.0/oc.txt " + "  "+ ResultDir + oneTest.replace(".tcl", "_StartWith_"+str(value)+".result")
		print CMD
		os.system(CMD)
...
def excute2():
	...
	for config in ConfSpecList:
		...
		for onetest in TestSuitList:
			...
			onetestname = onetest.replace(".tcl","")
